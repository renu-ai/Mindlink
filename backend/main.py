...
async def chat(request: Request):
    ...
    try:
        ...
        if len(messages) > MAX_HISTORY_LENGTH:
            if session["current_state"] == "chat":
                session["history"] = messages[-MAX_HISTORY_LENGTH:]
            elif session["current_state"] == "puzzle":
                session["puzzle_history"] = messages[-MAX_HISTORY_LENGTH:]
...
async def generate_puzzle(request: Request):
    ...
    try:
        ...
        if len(session["puzzle_history"]) > MAX_HISTORY_LENGTH:
            session["puzzle_history"] = session["puzzle_history"][-MAX_HISTORY_LENGTH:]
